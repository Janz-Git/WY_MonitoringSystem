...
namespace DiBo.DAL
{
    ...
    public partial class DVSAlarmInfoReport
    {
		...
		/// 增加一条数据
		/// </summary>
		public static int Add(DiBo.Model.DVSAlarmInfoReport model)
        {
            StringBuilder strSql = new StringBuilder();
		...
		/// </summary>
		public static List<DiBo.Model.DVSAlarmInfoReport> GetModels(int dataNumber)
        {
            List<DiBo.Model.DVSAlarmInfoReport> models = new List<Model.DVSAlarmInfoReport>();
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public static DiBo.Model.DVSAlarmInfoReport DataRowToModel(DataRow row)
        {
            DiBo.Model.DVSAlarmInfoReport model = new DiBo.Model.DVSAlarmInfoReport();
            if (row != null)
            {
        ...
        /// 获取记录总数
        /// </summary>
        public static int GetRecordCount(string startDate, string endDate)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM DVSAlarmInfoReport where left(CreateDate,10)>='" + startDate + "' and left(CreateDate,10)<='" + endDate + "' ");
        ...
        /// 
        /// </summary>
        /// <param name="startDate">起始日期 xxxx-xx-xx </param>
        /// <param name="endDate">截止日期 xxxx-xx-xx</param>
        /// <param name="currentPageIndex">当前页，从1开始</param>
        /// <param name="pageSize">每页大小</param>
        /// <returns></returns>
        public static DataTable GetList(string startDate, string endDate, int currentPageIndex, int pageSize = 15)
        {
            if (startDate.Length != 10)
                startDate = Convert.ToDateTime(startDate).ToString("yyyy-MM-dd");
            if (endDate.Length != 10)
            ...
            string strWhere = "";//不带where的查询条件，如果不为空，必须以 and 开头

            strWhere += " and left(CreateDate,10)>='" + startDate + "' and left(CreateDate,10)<='" + endDate + "' ";

            string strOrder = " CreateDate desc";//排序字段，注意只是字段名，例如：user_id desc/asc

            int recordCount = 0;

            if (currentPageIndex >= 0)
                return DbHelperMySQL.GetRecordByPage(out recordCount, "*", "DVSAlarmInfoReport", strWhere, strOrder, currentPageIndex, pageSize);
            else
                return DbHelperMySQL.ExecuteDataTable("select * from DVSAlarmInfoReport where left(CreateDate,10)>='" + startDate + "' and left(CreateDate,10)<='" + endDate + "' order by " + strOrder);
        }
