...
namespace DiBo.DAL
{
    ...
    public partial class DVSBasicParamsReturn
    {
        ...
        public static Model.DVSBasicParamsReturn SaveDVSBasicParamsReturn(Model.DVSBasicParamsReturn basicParams)
        {
            Model.DVSBasicParamsReturn model = basicParams;

            var obj = DbHelperMySQL.ExecuteScalar("select ParamsID from DVSBasicParamsReturn where DeviceID='" + basicParams.DeviceID + "'");
            if (obj != null)
            {
		...
		/// 增加一条数据
		/// </summary>
		public static int Add(DiBo.Model.DVSBasicParamsReturn model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into dvsbasicparamsreturn(");
		...
		/// 更新一条数据
		/// </summary>
		public static bool Update(DiBo.Model.DVSBasicParamsReturn model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update dvsbasicparamsreturn set ");
        ...
        public static DataTable GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM DVSBasicParamsReturn ");
            if (strWhere.Trim() != "")
            {
		...
		/// 得到一个对象实体
		/// </summary>
		public static DiBo.Model.DVSBasicParamsReturn GetModel(string strWhere="")
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select   ParamsID,CommandCode,DeviceID,DMAID,TotalPoints,FrameNumber,AvgNumber,HighpassHz,Scanrate,PulseWidth,FiberLength1,FiberOffset1,Noiselevel1,FiberLength2,FiberOffset2,Noiselevel2,AlarmResolution,MinimumAlarmCnt,DurationCnt,DealingWind,WindWindow,WindMinimumCnt,CreateDate,UpdateDate from DVSBasicParamsReturn ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 得到一个对象实体
        /// </summary>
        public static DiBo.Model.DVSBasicParamsReturn DataRowToModel(DataRow row)
        {
            DiBo.Model.DVSBasicParamsReturn model = new DiBo.Model.DVSBasicParamsReturn();
            if (row != null)
            {
                ...
                {
                    model.HighpassHz = decimal.Parse(row["HighpassHz"].ToString());
                }
                if (row["Scanrate"] != null && row["Scanrate"].ToString() != "")
                {
                    model.Scanrate = decimal.Parse(row["Scanrate"].ToString());
                }
